      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        ...
        <main className="flex-1 pb-24 lg:pb-8 overflow-x-hidden">
          ...
          >
            ...
            {/* Level Progress */}
            {currentUser && (
              <div className="card-premium rounded-2xl p-6 mb-8 shadow-glow-purple">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <div className="text-3xl font-bold text-text font-heading">Nível {currentUser.level}</div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {currentUser?.badges.filter(b => b.unlocked).map((badge, index) => (
                  <motion.div
                    ...
                    className="card-premium rounded-2xl p-4 text-center shadow-glow-purple"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {currentUser?.badges.filter(b => !b.unlocked).map((badge, index) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ delay: index * 0.1 }}
                    className="card-premium rounded-2xl p-4 text-center opacity-60"
                  >
